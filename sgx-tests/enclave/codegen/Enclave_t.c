...
static sgx_status_t SGX_CDECL sgx_run_tests_ecall(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_run_tests_ecall_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_run_tests_ecall_t* ms = SGX_CAST(ms_run_tests_ecall_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = run_tests_ecall();
...
SGX_EXTERNC const struct {
	...
	3,
	{
		{(void*)(uintptr_t)sgx_run_tests_ecall, 0, 0},
